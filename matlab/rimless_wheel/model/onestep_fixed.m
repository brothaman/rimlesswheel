%===================================================================
function [z,t,thetadotmid,Avg_Velocity,error_flag]=onestep_fixed(z0,parms,steps)
%===================================================================
...
%===================================================================
q = z(1); u = z(2);
M = parms.M; L = parms.L; g = parms.g; l = parms.l; %n = parms.n;
...
%%%%% 9/28 %
M1 = l*(M*l + l*m - L*M*cos(alpha - q));
RHS1 = L*M*l*u^2*sin(alpha - q) - M*g*l*sin(gam - q) - g*l*m*sin(gam - q) - L*M*g*sin(alpha - gam);
...
alpha = parms.control.alpha;
M = parms.M; L = parms.L; n = parms.n; l = parms.l; %g = parms.g; %gam = parms.gam;
m = parms.m;
...
%%%%%% 9/28
M1 = M*l^2 + l^2*m - L*M*l*cos(alpha - q);
RHS1 = m*(l^2*v*cos(r + (2*pi)/n)*cos(r) + l^2*v*sin(r + (2*pi)/n)*sin(r)) - M*(l*v*cos(r)*(L*cos(alpha) - l*cos(r + (2*pi)/n)) + l*v*sin(r)*(L*sin(alpha) - l*sin(r + (2*pi)/n)));
X = M1\RHS1;
...
%===================================================================
%q = z(:,1); %store angle
M = parms.M; m = parms.m; l = parms.l; g=parms.g; gam = parms.gam;
flag = 0;
qmin = min(z(:,1));
qmax = max(z(:,1));
...
for i=1:length(Fy)
    ...
    Fy(i,1) = (M + m)*(g + l*ud*sin(gam - q) - l*u^2*cos(gam - q));
end
